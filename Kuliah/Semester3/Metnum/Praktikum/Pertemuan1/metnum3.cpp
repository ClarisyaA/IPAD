...
double taylorSin(double x, double toleransi, int& iterasiTerakhir, double& galatRelatifIterasi, double& galatAbsolutIterasi) {
    double suku = x; 
    ...
    while (mutlak(suku) > toleransi && n < 5) {
        ...
        // Hitung galat absolut antar iterasi
        galatAbsolutIterasi = mutlak(hasilSin - hasilIterasiSebelumnya);
...
// Fungsi untuk menghitung galat absolut
double hitungGalatAbsolut(double nilaiAproksimak, double nilaiSebenarnya) {
    return mutlak(nilaiAproksimak - nilaiSebenarnya);
}
...
int main() {
    ...
    double galatAbsolutIterasi = 0.0; // Variabel untuk menyimpan galat absolut antar iterasi
    double hasilSin = taylorSin(x, toleransi, iterasiTerakhir, galatRelatifIterasi, galatAbsolutIterasi);

    // Hitung nilai sinus sebenarnya menggunakan deret Taylor dengan 10 suku
    ...
    cout << "Galat absolut antar iterasi: " << galatAbsolutIterasi << endl; // Tampilkan galat absolut antar iterasi
